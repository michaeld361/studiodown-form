...
const STEPS = [
  { id: "setup", stage: "01", title: "The basics.", subtitle: "Tell us about your company.", fields: [
    { key: "companyName", label: "Company name", type: "text", placeholder: "Acme Inc.", required: true },
    { key: "whatYouDo", label: "What does your company do?", type: "textarea", placeholder: "Describe it in one or two sentences, as if you're explaining it to a friend.", required: true },
    { key: "audience", label: "Who are your customers?", type: "textarea", placeholder: "e.g. Series A fintech founders, independent coffee shops, first-time homebuyers...", required: true },
    { key: "stage", label: "Where are you at?", type: "select", options: ["Pre-launch — we're still building", "Just launched — first customers", "Growing — product works, need to look the part", "Established — time for a rebrand"], required: true },
    { key: "goals", label: "What does success look like?", type: "textarea", placeholder: "e.g. A site that converts visitors into demo requests, looking premium enough for enterprise clients, a brand our team is proud to share...", required: true },
  ]},
  { id: "personality", stage: "02", title: "Personality.", subtitle: "If your brand walked into a room, what would people notice?", fields: [
    { key: "brandPerson", label: "Describe your brand as a person", type: "textarea", placeholder: "e.g. Calm, confident, slightly nerdy. The kind of person who explains complex things simply and always picks up the bill.", required: true },
    { key: "threeWords", label: "Three words that describe your brand", type: "text", placeholder: "e.g. precise, warm, bold", required: true },
    { key: "notThisWords", label: "Three words your brand is NOT", type: "text", placeholder: "e.g. corporate, playful, cheap", required: true },
  ...
  { id: "references", stage: "03", title: "References.", subtitle: "Show us what good looks like — and what you're up against.", fields: [
    { key: "admiredBrands", label: "2–3 brands or websites you admire visually", type: "textarea", placeholder: "e.g. Stripe's clean layout, Aesop's earthy warmth, Apple's restraint. Tell us what you like about each.", required: true },
    { key: "competitors", label: "Who are your main competitors?", type: "textarea", placeholder: "Names or URLs. We want to understand who you sit alongside — and how to make you stand out.", required: false },
    { key: "avoidBrands", label: "Anything you want to avoid?", type: "textarea", placeholder: "e.g. No gradients, nothing that looks like a SaaS template, avoid purple...", required: false },
  ]},
  { id: "existing", stage: "04", title: "What exists.", subtitle: "Help us understand what you're working with today.", fields: [
    { key: "existingBranding", label: "Do you have existing branding?", type: "select", options: ["Nothing — starting from scratch", "Logo only", "Logo + some colours/fonts", "Full brand guidelines (that we're replacing)"], required: true },
    { key: "existingUrl", label: "Current website (if any)", type: "text", placeholder: "https://", required: false },
    { key: "keepOrKill", label: "What do you love about your current brand? What do you hate?", type: "textarea", placeholder: "e.g. Love the logo, hate the colours. The tone of voice is right but the site feels dated. Keep the name, change everything else.", required: false },
    { key: "contentReady", label: "What content do you have ready?", type: "multiselect", options: ["Professional photography", "Product shots", "Written copy", "Video content", "Team headshots", "Nothing yet — need guidance"], required: true },
  ]},
  { id: "sensory", stage: "05", title: "Sensory.", subtitle: "These feel abstract, but they unlock something. Trust the process.", fields: [
    { key: "material", label: "If your brand were a material, what would it be?", type: "text", placeholder: "e.g. brushed concrete, worn leather, frosted glass, raw linen...", required: false },
    { key: "timeOfDay", label: "Time of day?", type: "select", options: ["Early morning — quiet, possibility", "Mid-morning — bright, energetic", "Golden hour — warm, confident", "Late evening — intimate, focused", "Midnight — bold, dramatic"], required: false },
    { key: "season", label: "Season?", type: "select", options: ["Spring — fresh, new, light", "Summer — bold, vibrant, warm", "Autumn — rich, mature, textured", "Winter — minimal, stark, precise"], required: false },
    { key: "anythingElse", label: "Anything else that captures the feeling?", type: "textarea", placeholder: "A song, a place, a photograph, a memory. Anything goes.", required: false },
  ]},
  { id: "scope", stage: "06", title: "The build.", subtitle: "What are we actually making?", fields: [
    ...
    { key: "functionality", label: "Any specific features?", type: "multiselect", options: ["E-commerce / shop", "Booking system", "Blog / journal", "Portfolio / case studies", "Contact / enquiry form", "Newsletter signup", "User accounts / login", "None of the above — just a clean site"], required: true },
    ...
    { key: "timeline", label: "When do you need this?", type: "select", options: ["Yesterday", "Within 2 weeks", "Within a month", "No rush — doing it right matters more"], required: true },
  ]},
      ...
      {view === "done" && (
        <div style={{
          ...
          <div style={{ maxWidth: 480 }}>
            ...
            <p style={{ fontSize: 15, color: "#666", lineHeight: 1.7, letterSpacing: "0.01em", whiteSpace: "pre-line" }}>
          ...
          <div style={{
            ...
            <a href="mailto:hello@studiodown.com" style={{ color: "#444", fontSize: 13, fontFamily: "'Outfit', sans-serif", letterSpacing: "0.1em", textDecoration: "none" }}>hello@studiodown.com</a>
          </div>
        </div>
      )}
    </div>
